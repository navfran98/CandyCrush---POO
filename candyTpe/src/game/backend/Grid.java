...
public abstract class Grid {
	...
	protected abstract void setSpecialElements();
	public abstract void removeSpecialElements(Element element);
	...
	private void removeFigure(int i, int j, Figure f) {
		...
		Element element = g[i][j].getContent();
		if (f.hasReplacement()) {
			// If a special element is found, and we have to ensure that we remove it correctly
			if(element.isSpecial() && element.isExplodable()){
				state().addSpecialElementRemoved();
				removeSpecialElements(element);
			}
