...
public class CandyFrame extends VBox {
	...
	public CandyFrame(CandyGame game) {
		...
		game.initGame();
		levelDataPanel = new LevelDataPanel(game);
		getChildren().add(levelDataPanel);
		...
		addEventHandler(MouseEvent.MOUSE_CLICKED, event -> {
			if (lastPoint == null) {
				...
				if (newPoint != null) {
					...
					if (game().isFinished()) {
						if (game().playerWon()) {
							message = game().getCurrentState() + " Finished - Player Won!";
						} else {
							message = game().getCurrentState() + " Finished - Loser !";
						}
					} else {
						message = game().getCurrentState();
					}
					levelDataPanel.updateScore(message);
					lastPoint = null;
				}
			}
		});
